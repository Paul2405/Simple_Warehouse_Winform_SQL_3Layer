...
namespace Project
{
    public partial class FrmExport : Form
    {
        ...
        private void DgvExport_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("List Product is empty", "Warning");
            }
            else
            {
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            if (index == -1)
            {
                MessageBox.Show("Please choose a product to export", "Warning");
            }
            else
            {
                if (validate())
                {
                    ...
                    {
                        MessageBox.Show("please input a number of package or dub package to export", "Warning");
                        return;
                    }
                    if (pricePackageExport != 0)
                    {
                        if (numSub == 0)
                        {
                            ...
                            if (numPack <= currentNumberPackage && numSub <= currentNumSub)
                            {
                                ...
                                {
                                    MessageBox.Show("Add to export list successful", "Notification");
                                    listExport.Add(ep);
                                    index = -1;
                                    txtProductID.Text = "";
                                    txtProductName.Text = "";
                                ...
                                {
                                    MessageBox.Show("Add to export list fail", "Notification");
                                    index = -1;
                                    txtProductID.Text = "";
                                    txtProductName.Text = "";
                                    txtDesCription.Text = "";
                            ...
                            {
                                MessageBox.Show("Number export greater than number produts have been, please inmput again", "Warning");
                            }
                        }
                        else
                        {
                            if (priceSubExport != 0)
                            {
                                if(numPack == currentNumberPackage)
                                {
                                   if(numSub == 0)
                                    {
                                        Export ep = new Export();
                                   ...
                                   else if (numSub <= (currentNumSub - currentCount))
                                    {
                                        Export ep = new Export();
                                        ep.ExportID = userID;
                                        ep.ProductID = productID;
                                        ...
                                        bool checkUpdateProdutc = ProDAO.updateProductAtCountExport((numSub + currentCount - currentNumSub), currentNumberPackage - numPack, productID);
                                        bool checkInsertExport = exportDAO.insertExport(ep);
                                        if (checkUpdateProdutc && checkInsertExport)
                                        {
                                            MessageBox.Show("Add to export list successful", "Notification");
                                    ...
                                    {
                                        MessageBox.Show("Number product out of current number in the warehouse", "warning");
                                    }
                                }else if (numSub <= currentNumSub && numPack < currentNumberPackage)
                                {
                                    Export ep = new Export();
                                    ep.ExportID = userID;
                                    ep.ProductID = productID;
                                    ...
                                    if (numSub + currentCount > currentNumSub)
                                    {
                                        ...
                                        {
                                            MessageBox.Show("Add to export list fail", "Warning");
                                            index = -1;
                                            txtProductID.Text = "";
                                            txtProductName.Text = "";
                                            txtDesCription.Text = "";
                                ...
                                {
                                    MessageBox.Show("Please input number sub package less than number sub of one package or yout can export a package", "Warning");
                                }

                            }
                            else
                            {
                                MessageBox.Show("Price to export had not been input, please input at manage product function", "Warning");
                            }
                        }
                    }
                    else
