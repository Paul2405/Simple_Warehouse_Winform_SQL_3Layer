...
namespace ManageDAO
{
    ...
    public class ProDAO
    {
        ...
        public static bool updatePriceExportByID(string id, float pricePackage, float priceSub, string name, string description)
        {
            bool check = false;
            SqlConnection conn = new SqlConnection(connStr);
            SqlCommand cmd = new SqlCommand("Update Product set PriceToExportPackage = @PriceToExportPackage, PriceToExportSub = @PriceToExportSub , ProductName = @ProName , Description = @Description where ProductID = @ProductID", conn);
